    ...
    <div class="bg-gray-800 pt-2 md:pt-1 pb-1 px-1 mt-0 h-auto fixed w-full z-20 top-0">
        <div class="flex flex-wrap items-center">
            <div class="flex flex-shrink md:w-1/3 justify-center md:justify-start text-white">
                <a href="{{ route('dashboard') }}" aria-label="Home">
                    @if(Storage::disk('public')->exists('logo.png'))
                        <img src="{{ asset('storage/logo.png') }}" class="max-h-9 max-w-xs pl-2 mt-1" alt="{{ __('header.logo') }}">
                    @else
            ...
            <div class="flex w-full pt-2 content-center justify-between md:w-1/3 md:justify-end">
                <ul class="list-reset flex justify-between flex-1 md:flex-none items-center greeting-wrap">
                    <li class="flex items-center flex-1 md:flex-none md:mr-3">
                        <div class="relative inline-block rounded-full overflow-hidden ml-2 user-photo">
                            <img src="{{ Auth::user()->photoSrc() }}" alt="">
                        </div>
