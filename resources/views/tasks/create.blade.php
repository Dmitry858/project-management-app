...
@section('content')
    {{ Breadcrumbs::render('create-task') }}
    @include('include.flash-error')

    <form class="w-full max-w-lg" method="POST" action="{{ route('tasks.store') }}" enctype="multipart/form-data">
        @csrf
        <div class="flex flex-wrap -mx-3 mb-6">
            <div class="w-full px-3 mb-6">
                <label class="flex uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="name">
                    @lang('form.label_title')
                    @include(
                        'include.field-tooltip',
                        ['text' => __('form.max_length_text', ['length' => 180])]
                    )
                </label>
            ...
            <div class="w-full px-3 mb-6">
                <label class="flex uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="description">
                    @lang('form.label_description')
                    @include(
                        'include.field-tooltip',
                        ['text' => __('form.max_length_text', ['length' => 2000])]
                    )
                </label>
                <textarea class="appearance-none block w-full text-gray-700 border border-gray-300 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 h-48" id="description" name="description" type="text" value=""></textarea>
            </div>
